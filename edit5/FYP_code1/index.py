...
def register():
    ...
    if request.method == 'POST' and len(dict(request.form)) > 0:
        ...
        try:
            ...
            doc_ref=firestore_db.collection(u'users').document(userID)
            #print(doc_ref)
            doc_ref_set=doc_ref.set({'email': email, 'car': car,'model':model,'password':password})
            #print(doc_ref,doc_ref_set)
            sub_ref=doc_ref.collection(u'livedata').document(u'samplelive').set({'IntakeAirTemp': "",'EngineRPM':'','CoolantTemperature':""})
            #print(sub_ref)
            return render_template("index.html")
        except:
...
def checkhealth():
    ...
    return render_template("userhomepage.html", coolant_temp=send_list_values[6], avg_speed=send_list_values[8],
                           ...
                           vehicle_speed=send_list_values[5],res=res,res_word=res_word,type='Health')
...
def livecheckhealth():
    ...
    return render_template("userhomepage.html", coolant_temp=send_list_values[6], avg_speed=send_list_values[8],
                           intake_air_temp=send_list_values[7], userDetails=userDetails, list_values=send_list_values,
                           engine_rpm=send_list_values[3], engine_runtime=send_list_values[4],
                           vehicle_speed=send_list_values[5],res=res,res_word=res_word,type='Live Health')
